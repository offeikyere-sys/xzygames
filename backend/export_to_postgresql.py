    ...
    with open(OUTPUT_FILE, 'w', encoding='utf-8') as f:
        ...
        for movie in movies:
            ...
            values = {
                ...
                'cast_name': (movie_dict.get('cast') or movie_dict.get('cast_name') or '').replace("'", "''"),
            ...
            # Handle NULL year (None -> NULL) for PostgreSQL
            year_sql = 'NULL' if values['year'] is None else str(values['year'])
            f.write(f"({values['id']}, '{values['title']}', '{values['genre']}', {year_sql}, '{values['duration']}', ")
